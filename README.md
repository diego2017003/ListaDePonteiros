# ListaDePonteiros
lista de ponteiros
Meu nome é *Diego Rodrigues Medeiros* e este repositório é destinado à resolução das questões da lista de ponteiros 
...
int main(){
  int a[3]={6,5,6},b[3]={5,3,4},r[3];//exemplo
  somaVetor(r,a,b,3);//chamdno a função
  for(int i=0;i<3;i++){
...
```
## Questão 19
### Multiplicando matrizes utilizando alocação dinâmica
*resp:*
```c
  #include<stdio.h>
#include<stdlib.h>
    ...
    for(int i=0; i<nla; i++) //sinaliza a linha da matriz r
    {
        for(int j=0; j<ncb; j++) //sinaliza a coluna da matriz r
        {
            *(r+j+i*ncb)=0;//inicializa o elemento da matriz r como 0 para possibilitar futura soma de valores
            ...
            {
                *(r+j+i*ncb)+=(*(a+k+i*nca))*(*(b+k*ncb+j));//a multiplcação dos termos e a soma das multiplicações
            }
        }
    }
}
